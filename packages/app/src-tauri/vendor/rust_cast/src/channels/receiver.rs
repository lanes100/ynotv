        ...
        self.message_manager.receive_find_map(|message| {
            ...
            match self.parse(message)? {
                ReceiverResponse::Status(mut status) => {
                    let matching_app_position = status
                        .applications
                        .iter()
                        .position(|application| application.app_id == app_id);

                    if status.request_id == request_id || matching_app_position.is_some() {
                        if let Some(position) = matching_app_position
                            .or_else(|| (!status.applications.is_empty()).then_some(0))
                        {
                            return Ok(Some(status.applications.remove(position)));
                        }

                        log::debug!(
                            "Receiver status for launch request {request_id} did not include applications yet."
                        );
                    }
                }
