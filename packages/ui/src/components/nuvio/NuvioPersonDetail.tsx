...
interface NuvioPersonDetailProps {
  personId: number;
  onBack: () => void;
...
export function NuvioPersonDetail({ personId, onBack, onItemClick }: NuvioPersonDetailProps) {
  ...
  useEffect(() => {
    ...
    getPersonDetails(tmdbToken, personId)
      ...
      .catch((err) => {
        console.error('[NuvioPersonDetail] Failed to load person details:', err);
        if (cancelled) return;
        setLoading(false);
      });
  ...
  return (
    <div className="nuvio-person-detail" ref={scrollRef}>
      {/* Dynamic blurred backdrop bleed */}
      {backdrop && (
        <div className="nuvio-person-backdrop-bleed">
          <div
      ...
      {/* Main Container */}
      <div className="nuvio-person-content">
        {/* Navigation */}
        <button className="nuvio-person-back-btn" onClick={onBack}>
        ...
        {/* Profile Header */}
        <div className="nuvio-person-header">
          <div className="nuvio-person-avatar-wrap">
            <Poster src={photo || undefined} seed={person.name} />
          </div>

          <div className="nuvio-person-info">
